...
# run the model for a policy scenario and return group data
def run_model(policy_name, insurance=False, subsidy=False, nudge=False, steps=30, seeds = range(100), sensitivity_params = None):
    ...
    # create and run the model
            ...
            df_hist = model.get_group_history()
...
if __name__ == "__main__":
    ...
    df_cost_all = pd.concat([df_none, df_ins, df_sub, df_nudge], ignore_index=True)
    df_hist = pd.concat([df_hist_none, df_hist_ins, df_hist_sub, df_hist_nudge], ignore_index=True)



    output_dir = Path(__file__).with_name("results")    
    output_dir.mkdir(exist_ok=True)    
    output_path = output_dir / "group_historyE4.csv"    
